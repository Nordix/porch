...
// It uses round-robin among ready pods with equal load to ensure even distribution.
...
func (pcm *podCacheManager) findBestPod(fn *functionInfo) (int, int) {
	...
	for i := 1; i < n; i++ {
		if wl := fn.pods[i].WaitlistLen(); wl < minWaitlist {
			minWaitlist = wl
		}
	}
	...
	for i := 0; i < n; i++ {
		...
		if fn.pods[idx].WaitlistLen() == minWaitlist {
			fn.roundRobinIdx = idx + 1
			return idx, minWaitlist
		}
	}

	return 0, minWaitlist
}
...
func NewPodInfo(firstResponseCh chan<- *connectionResponse) functionPodInfo {
	pod := functionPodInfo{
		...
		concurrentEvaluations: &atomic.Int32{},
		// fnEvaluationMutex removed - no longer needed after removing serialization
	}
