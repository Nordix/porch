...
func TestReplaceNonKRMFiles(t *testing.T) {
	for i := range nonKRMTests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			err = copyutil.CopyDir(filesys.FileSystemOrOnDisk{}, filepath.Join(ds, test.updated), updated)
			assert.NoError(t, err)
			err = copyutil.CopyDir(filesys.FileSystemOrOnDisk{}, filepath.Join(ds, test.original), original)
			assert.NoError(t, err)
			err = copyutil.CopyDir(filesys.FileSystemOrOnDisk{}, filepath.Join(ds, test.local), local)
			assert.NoError(t, err)
			err = copyutil.CopyDir(filesys.FileSystemOrOnDisk{}, filepath.Join(ds, test.expectedLocal), expectedLocal)
			assert.NoError(t, err)
