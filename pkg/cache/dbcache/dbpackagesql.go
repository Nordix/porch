...
func pkgReadFromDB(ctx context.Context, pk repository.PackageKey) (dbPackage, error) {
	...
	err := dbhandler.GetDB().Db.QueryRow(sqlStatement, pk.GetRepositoryKey().Namespace, pk.GetRepositoryKey().Name, pk.Package).Scan(
...
func pkgReadPkgsFromDB(ctx context.Context, rk repository.RepositoryKey) ([]dbPackage, error) {
	...
	rows, err := dbhandler.GetDB().Db.Query(
...
func pkgWriteToDB(ctx context.Context, p *dbPackage) error {
	...
	if _, err := dbhandler.GetDB().Db.Exec(
		sqlStatement, pk.GetRepositoryKey().Namespace, pk.GetRepositoryKey().Name, pk.Package, valueAsJson(p.meta), valueAsJson(p.spec), p.updated, p.updatedBy); err == nil {
...
func pkgUpdateDB(ctx context.Context, p *dbPackage) error {
	...
	if _, err := dbhandler.GetDB().Db.Exec(
		sqlStatement,
		pk.GetRepositoryKey().Namespace, pk.GetRepositoryKey().Name, pk.Package, valueAsJson(p.meta), valueAsJson(p.spec), p.updated, p.updatedBy); err == nil {
...
func pkgDeleteFromDB(ctx context.Context, pk repository.PackageKey) error {
	...
	if _, err := dbhandler.GetDB().Db.Exec(sqlStatement, pk.GetRepositoryKey().Namespace, pk.GetRepositoryKey().Name, pk.Package); err == nil {
