...
func pkgReadPkgsFromDB(ctx context.Context, rk repository.RepositoryKey) ([]*dbPackage, error) {
	...
	rows, err := dbhandler.GetDB().Db.Query(sqlStatement, rk.Namespace, rk.Name)
	if err != nil {
		return nil, err
	}
...
func pkgWriteToDB(ctx context.Context, p *dbPackage) error {
	...
	pk := p.Key()
	if _, err := dbhandler.GetDB().Db.Exec(sqlStatement,
		pk.K8SNS(), pk.K8SName(),
...
func pkgDeleteFromDB(ctx context.Context, pk repository.PackageKey) error {
	...
	if _, err := dbhandler.GetDB().Db.Exec(sqlStatement, pk.K8SNS(), pk.K8SName()); err == nil {
