...
func (pr *dbPackageRevision) GetPackageRevision(ctx context.Context) (*porchapi.PackageRevision, error) {
	...
	// Only re-read from DB if we don't have cached Kptfile (not from List operation)
...
func (pr *dbPackageRevision) GetKptfile(ctx context.Context) (kptfile.KptFile, error) {
	...
	var kfString string
	if len(pr.resources) > 0 {
		if cached, ok := pr.resources[kptfile.KptFileName]; ok {
			kfString = cached
		}
	}

	// Fall back to DB read if not cached
	if kfString == "" {
		var err error
