...
func (r *dbRepository) DeletePackageRevisionDraftFromDB(ctx context.Context, newPR *v1alpha1.PackageRevision) error {
	klog.Infof("DB Repo DeletePackageRevisionDraft: %q", r.Key().String())

	_, span := tracer.Start(ctx, "dbRepository::DeletePackageRevisionDraft", trace.WithAttributes())
	defer span.End()

	prk := repository.PackageRevisionKey{
		PkgKey: repository.PackageKey{
			RepoKey: repository.RepositoryKey{
				Namespace: newPR.Namespace,
				Name:      newPR.Spec.RepositoryName,
			},
			Package: newPR.Spec.PackageName},
		Revision:      newPR.Spec.Revision,
		WorkspaceName: newPR.Spec.WorkspaceName,
	}

	if err := pkgRevDeleteFromDB(ctx, prk); err != nil && err != sql.ErrNoRows {
		klog.Warningf("dbPackage:DeletePackageRevision: deletion of %q failed on database %q", prk.String(), err)
	}
	return nil
}
