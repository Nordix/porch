...
func (ah *AsyncHandler) SavePackageRevisionJob(ctx context.Context, cacheOpts cachetypes.CacheOptions, prk repository.PackageRevisionKey, status string) error {
	...
	if cacheOpts.CacheType == "DB" {
		...
		sqlStatement := `
        INSERT INTO pr_job (name_space, repo_name, package_name, package_rev, workspace_name, status)
		...
		if _, err := dbhandler.GetDB().Db.Exec(
			sqlStatement,
			prk.GetPackageKey().GetRepositoryKey().Namespace, prk.GetPackageKey().RepoKey.Name, prk.GetPackageKey().Package, prk.Revision, prk.WorkspaceName, status); err == nil {
...
func (ah *AsyncHandler) DeletePackageRevisionJob(ctx context.Context, cacheOpts cachetypes.CacheOptions, prk repository.PackageRevisionKey) error {
	...
	if cacheOpts.CacheType == "DB" {
		...
		sqlStatement := `
        DELETE FROM pr_job WHERE name_space=$1 AND repo_name=$2 AND package_name=$3 AND package_rev=$4 AND workspace_name=$5`

		if _, err := dbhandler.GetDB().Db.Exec(
			sqlStatement,
			prk.GetPackageKey().GetRepositoryKey().Namespace, prk.GetPackageKey().RepoKey.Name, prk.GetPackageKey().Package, prk.Revision, prk.WorkspaceName); err == nil {
...
func (ah *AsyncHandler) ListPackageRevisionJobs(ctx context.Context, cacheOpts cachetypes.CacheOptions, prk repository.PackageRevisionKey) ([]*Job, error) {
	...
	if cacheOpts.CacheType == "DB" {
		...
		sqlStatement := `SELECT * FROM pr_job WHERE name_space=$1 AND repo_name=$2`
		tasks := make([]*Job, 0)
		if rows, err := dbhandler.GetDB().Db.Query(
			sqlStatement,
			prk.GetPackageKey().GetRepositoryKey().Namespace, prk.GetPackageKey().RepoKey.Name); err == nil {
