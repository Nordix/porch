...
func (r *packageCommon) updatePackageRevision(ctx context.Context, name string, objInfo rest.UpdatedObjectInfo,
	...
	go r.asyncUpdatePackageRevision(name, ns, createValidation, updateValidation, objInfo, forceAllowCreate)

	newRuntimeObj := api.PackageRevision{}
	newRuntimeObj.Name = name
	newRuntimeObj.Namespace = ns

	// Return the context created by the apiserver
	return &newRuntimeObj, true, nil
}

func (r *packageCommon) asyncUpdatePackageRevision(name, ns string, createValidation rest.ValidateObjectFunc, updateValidation rest.ValidateObjectUpdateFunc, objInfo rest.UpdatedObjectInfo, forceAllowCreate bool) {
	...
	repoName, err := util.ParsePkgRevObjNameField(name, 0)
	if err != nil {
		err := apierrors.NewBadRequest(fmt.Sprintf("invalid name %q", name))
	...
	if isCreate {
		r.savePkgRevJobInDB(goCtx, newApiPkgRev, "Package revision create in progress")
	...
	r.savePkgRevJobInDB(goCtx, newApiPkgRev, "Package revision updated")
	goCtx.Done()
}
