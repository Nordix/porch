...
Supported Flags:
  ...
  --enabled-reconcilers RECONCILDERS  ... comma-separated list of reconcilers that should be enabled in
                                          porch controller
  --krm-fn-image-prefix PREFIX           ... gcr image url prefix for running porch behind a proxy
EOF
  exit 1
}
...
for resource in ctx.resource_list['items']:
  ...
  for container in containers:
    container['args'].append('--default-image-prefix=${KRM_FUNCTION_IMAGE_PREFIX}')
"
}
...
function main() {
  ...
  if [[ -n "${KRM_FUNCTION_IMAGE_PREFIX}" ]]; then
